    ...
    for i in range(num):
        ...
        if index != -1:
            ...
            if count == 0:
                ...
                #print(latest_sentence[0][0:-4])

                url = 'http://www.rcp.keio.ac.jp/ora/jukunai/#one'
                res = requests.get(url)
                bsObj = bs4.BeautifulSoup(res.text.encode(res.encoding), "html.parser")
                parseURL = bsObj.select(".h2_area")
                # sentence = []
                for text in parseURL:
                    title = text.getText().strip()
                    # sentence.append(title)
                # for line in sentence:
                    string = title.encode('utf-8')
                    pattern = latest_sentence[0][0:-4]
                    matchOB = re.search(pattern,string)
                    if matchOB:
                        # string -> unicode
                        tmp = text.findNext('tbody').find('tr').findAll('td')
                        tweet_schedule = tmp[-1].getText()
                tweet_url = latest_sentence[1]
                #print tweet_schedule
                return [tweet_text,tweet_url,tweet_schedule]
...
def tweetMenu():
    ...
    api = tweepy.API(auth)
    # send tweet
    api.update_status(unicode(text,'utf-8') + url + schedule)
